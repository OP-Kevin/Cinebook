...
import { clearAuthSession, getAuthHeaders, getLoggedUser } from "@/utils/auth";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-black via-purple-900 to-black px-3 sm:px-6 py-6">
      ...
      <div className="mt-24 md:mt-16 flex justify-center items-start md:items-center min-h-[85vh]">
        <div className=" w-full max-w-md sm:max-w-2xl md:max-w-4xl lg:max-w-5xl bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl shadow-2xl p-5 sm:p-8 md:p-10 ">
          {/* ⭐ PROFILE HEADER */}
          {/* ⭐ PROFILE HEADER */}
              ...
              {history.map((item, i) => (
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    <FaFilm />
                    <p className="font-semibold">{item.movieName}</p>
                  </div>

                  <p className="text-sm">Theater: {item.theaterName}</p>
                  ...
                  <p className="font-bold mt-1">₹ {item.total}</p>
                </div>
              ))}
            </div>
          )}
